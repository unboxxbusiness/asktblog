...
import { Search, Sun, Moon, Bell, Download, Menu, X, ArrowRight, TrendingUp } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center gap-2.5 group">
            {/* Elegant Minimalist Growth Emblem (TrendingUp Arrow) */}
            <div className="relative h-9 w-9 rounded-xl bg-gradient-to-tr from-[#14213d] to-[#1d325c] dark:from-[#fca311] dark:to-[#ffbd44] flex items-center justify-center shadow-md shadow-primary/10 group-hover:scale-105 transition-transform duration-300">
              <TrendingUp className="w-5 h-5 text-white dark:text-[#14213d]" />
            </div>
            {/* Elegant Hybrid Logo Typography */}
            <span className="flex items-baseline font-heading tracking-tight select-none">
              <span className="font-serif italic font-normal text-foreground/90 text-2xl lowercase mr-0.5">the</span>
              <span className="font-black text-2xl text-primary uppercase tracking-tighter">askt</span>
            </span>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <Link
              href="/ai-news"
              className={`text-sm font-medium transition-colors hover:text-primary ${
                pathname === "/ai-news" ? "text-primary" : "text-muted-foreground"
              }`}
            >
              AI News
            </Link>
            <Link
              href="/ai-tools"
              className={`text-sm font-medium transition-colors hover:text-primary ${
                pathname === "/ai-tools" ? "text-primary" : "text-muted-foreground"
              }`}
            >
              AI Tools
            </Link>
      ...
      <div className="fixed inset-x-0 bottom-0 top-16 z-50 bg-background md:hidden animate-fade-in border-t border-border overflow-y-auto">
          <div className="flex flex-col p-6 gap-6">
            ...
            <Link
              href="/ai-news"
              className={`text-lg font-semibold border-b border-border pb-2 ${
                pathname === "/ai-news" ? "text-primary" : "text-foreground"
              }`}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              AI News
            ...
            <Link
              href="/ai-tools"
              className={`text-lg font-semibold border-b border-border pb-2 ${
                pathname === "/ai-tools" ? "text-primary" : "text-foreground"
              }`}
              onClick={() => setIsMobileMenuOpen(false)}
            >
              AI Tools
            ...
            <div className="flex flex-col gap-2">
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">LearnHub Categories</span>

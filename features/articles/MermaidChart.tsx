...
// Injects mermaid.min.js from jsDelivr exactly once per page.
// Uses a plain <script> tag — no Webpack dynamic import, no chunk splitting,
// no ./825.js errors. The CSP in next.config.mjs already allows cdn.jsdelivr.net.
...
function loadMermaid(): Promise<void> {
  ...
  mermaidPromise = new Promise<void>((resolve, reject) => {
    ...
    // Already loaded by a previous chart instance
    if ((window as any).mermaid?.render) {
      resolve();
      return;
    }

    // Check if a script tag is already in the DOM (e.g. hot-reload)
...
// ── Unique ID counter (multiple charts on same page) ─────────────────────────
...
export default function MermaidChart({ chart, category }: MermaidChartProps) {
  ...
  // Sync SVG width when zoom changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function render() {
      try {
        ...
        // ── Force vertical layout regardless of what is stored in DB ──────────
        // Replaces any `flowchart LR` or `graph LR` with `flowchart TD`
        // so the diagram is always rendered top-to-bottom.
        ...
        if (!cancelled) {
          ...
          // Reset singleton so a retry can re-load the script
          mermaidPromise = null;
        }
      }
    }

...
async function pingSitemaps() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://theaskt.org";
...
async function publish() {
  ...
  try {
    ...
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://theaskt.org";
    const articleFullUrl = `${siteUrl.replace(/\/$/, "")}/articles/${slug}`;

    const notifySecret = process.env.THEASKT_API_KEY;
    ...
    if (notifySecret) {
      ...
      console.warn("[!] Webhook triggers skipped: THEASKT_API_KEY missing.");
    }
    ...
    console.log("\n" + "═".repeat(60));
    console.log("  ✅ TheAskt Article Published Successfully");

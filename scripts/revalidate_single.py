...
def main():
    ...
    api_key = env.get('THEASKT_API_KEY', '')
    base_url = env.get('NEXT_PUBLIC_SITE_URL', 'https://theaskt.org')
    
    try:
        with open('draft_article.json', 'r') as f:
            draft = json.load(f)
        import re
        slug = re.sub(r'[^a-z0-9]+', '-', draft.get('title', '').lower()).strip('-')
        category = draft.get('category', 'Automation')
    except Exception as e:
        print("Could not read draft_article.json", e)
        return
